...
\revise{皮むき動作では，第1章の形状と力学という分析観点を，食材表面の法線・曲率に基づく動作軌道と，力覚フィードバックによる接触維持へ具体化する必要がある．}
...
本章は，家庭で野菜・果物の可食部を調理前に露出させる下処理を想定し，洗浄済みの一個の食材が与えられ，その中心軸に沿って串で支持できる状況を対象とする．
ロボットには「この食材のピーラーで到達可能な外皮を除去する」という操作目的，ピーラーの機能，食材の支持方法が与えられており，献立判断，食材選択，洗浄，芽取り，および処理後の安全・衛生管理は対象外とする．

従来の対象ごとのCADモデルまたは手作業による軌道教示を用いず，個体ごとに取得した表面形状からピーラーの到達可能軌道を生成し，モデル誤差や凹凸を力覚で補償しながら，設定した到達可能領域の外皮を除去することを達成目標とする．
食材別に異なる軌道を事前登録するのではなく，軌道生成，法線に基づく分割，力制御という主要処理は共通とする．
一方，処理済み領域を色差で識別できる食材では境界から回転角を更新し，色差が小さいにんじんでは固定回転角を用いるため，観測可能性に応じた分岐が必要である．
また，本実験では全食材に同一の目標接触力を用いており，硬さに応じた力の自動調整は達成範囲に含まれない．
\end{revisionblock}
...
\revise{これらは皮むき固有の幾何分析であり，物理的な接触を伴わない事前計算段階で，個体食材の形状から実行軌道と区間を導出する．}
...
\revise{これらの領域は，人間がりんごの皮を剥く場合にも，食べやすいサイズに切った後で追加処理されることがある．本結果では，ピーラーが到達可能な主要部分を処理できた一方，端部と支持部周辺の追加処理が必要である．}
...
\revise{この結果は，個体形状から生成した軌跡だけでは吸収できない実表面との差に対し，皮むき固有の力覚フィードバックを組み合わせる必要性を示す．}
...
\begin{revisionblock}
\subsubsection{家庭内の下処理作業としての評価}
家庭料理の目的から見ると，本実験で重要な結果は，形状と表面性状の異なる五種類の食材について，事前の精密な形状登録なしに皮むき動作を生成し，全試料で到達可能領域の78\%以上を処理できた点である．
特に，曲率の異なるりんご，きゅうり，なすに同じ軌道生成手順を適用し，じゃがいもの表面凹凸によるモデル誤差を力制御で補償できたことは，家庭に現れる個体差への対応可能性を示す．

一方，現状の結果だけで家庭用として実用的とは結論できない．
処理に約1時間を要し，りんごの端部，じゃがいもの芽・くぼみ，串周辺には剥き残しがあり，にんじんでは処理済み領域を自動判定できなかった．
さらに，食材へ串を刺す準備，剥いた皮の回収，刃物周辺の安全，衛生，清掃は評価していない．
したがって，本章が示したのは，家庭の下処理全体の自動化ではなく，所定の支持条件と到達可能範囲において，個体形状に応じたピーラー軌道と接触維持を自動生成できることである．
\end{revisionblock}
...
\revise{本章では，皮むき動作について，入力となる個体食材のRGB点群と既知の回転軸から，到達可能軌道，軌道区間，回転角および目標接触力を生成し，双腕ロボットの動作へ変換するタスク固有手法を構成した．}
...
\begin{revisionblock}
以上の結果から，対象ごとの事前軌道を用いず，個体形状から生成した軌道と力覚フィードバックを組み合わせることで，設定した支持・到達条件下の食材に対して78--95\%の皮むき率を得られることを示した．
ただし，くぼみ・支持部周辺の処理，色差が小さい食材の自動終了判定，食材硬さに応じた力調整，処理時間，安全・衛生を含む家庭下処理全体は未解決である．
このように，本章は第1章の形状・力学という分析観点を，表面法線，既処理境界，接触力という具体量へ落とし込み，皮むき固有の動作へ接続した事例である．
\end{revisionblock}
